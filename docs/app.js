(function(){
      ...
      function renderLists() {
        if (xmlFiles.length === 0) {
          ...
          xmlListContainer.innerHTML = xmlFiles.map(f => 
            `<div class="list-item"><span class="material-symbols-outlined">description</span>${escapeHtml(f.name)}</div>`
          ).join('');
        }
        ...
        if (pngFiles.length === 0) {
          ...
          pngListContainer.innerHTML = pngFiles.map(f => 
            `<div class="list-item"><span class="material-symbols-outlined">image</span>${escapeHtml(f.name)}</div>`
          ).join('');
        }
        updateStatus('files');
      }
      ...
      document.getElementById('selectOutputBtn').addEventListener('click', async () => {
        if (window.showDirectoryPicker) {
          ...
          const folderName = prompt('Enter output folder name:', 'optimized_output');
      ...
      function processXMLText(xmlText, division) {
        ...
        for (let el of subTextures) {
          ['x','y','width','height','frameX','frameY','frameWidth','frameHeight'].forEach(attr => {
        ...
        if (!result.startsWith('<?xml')) result = '<?xml version="1.0" encoding="UTF-8"?>\n' + result;
        return result;
      }
      ...
      async function saveFile(data, fileName, mimeType) {
        if (outputFolder && typeof outputFolder === 'object' && 'getFileHandle' in outputFolder) {
          try {
            ...
            const blob = typeof data === 'string' ? new Blob([data], { type: mimeType }) :
                         data instanceof Blob ? data : new Blob([data], { type: mimeType });
            await writable.write(blob);
            await writable.close();
            return;
          } catch (e) { console.warn('FS fallback', e); }
        }
        const blob = typeof data === 'string' ? new Blob([data], { type: mimeType }) :
                     data instanceof Blob ? data : new Blob([data], { type: mimeType });
      ...
      async function processFiles() {
        ...
        let xmlProcessed = 0, pngProcessed = 0, errors = [];

        try {
          for (let i = 0; i < xmlFiles.length; i++) {
            updateStatus('info', `XML ${i+1}/${xmlFiles.length}: ${xmlFiles[i].name}`);
            try {
              ...
              xmlProcessed++;
            } catch (e) { errors.push(`XML ${xmlFiles[i].name}: ${e.message}`); }
          }

          for (let i = 0; i < pngFiles.length; i++) {
            updateStatus('info', `Image ${i+1}/${pngFiles.length}: ${pngFiles[i].name}`);
            try {
              ...
              const resized = await resizePNG(arrayBuffer, img.width, img.height, newWidth, newHeight, useAA);
              await saveFile(resized, pngFiles[i].name, 'image/png');
              pngProcessed++;
            } catch (e) { errors.push(`PNG ${pngFiles[i].name}: ${e.message}`); }
          }

          let msg = `Done: ${xmlProcessed} XML, ${pngProcessed} PNG.`;
          if (errors.length) msg += ` Errors: ${errors.join('; ')}`;
          alert(errors.length ? 'Completed with errors.\n' + msg : 'Complete!\n' + msg);
